...
class UpdateNodeService < BaseService
  ...
  def process_features
    node.info.merge!({
      'emoji_reaction_type' => (
        if %w(pleroma_custom_emoji_reactions custom_emoji_reactions emoji_reaction).any? {|key| nodeinfo('metadata', 'features')&.include?(key)} || instance('fedibird_capabilities')&.include?('emoji_reaction') || info('upstream_name') == 'misskey'
          'custom'
