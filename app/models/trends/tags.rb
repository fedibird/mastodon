...
class Trends::Tags < Trends::Base
  ...
  def register(status, at_time = Time.now.utc)
    ...
    status.tags.each do |tag|
      next if tag.name.match?(TimeLimit::TIME_LIMIT_RE)
  ...
  def add(tag, account_id, at_time = Time.now.utc)
    trend_history(tag).add(account_id, at_time)
  ...
  def trend_history(tag)
    Trends::History.new('tags', tag.id)
  end
  ...
  def calculate_scores(tags, at_time)
    ...
    tags.each do |tag|
      expected  = trend_history(tag).get(at_time - 1.day).accounts.to_f
      expected  = 1.0 if expected.zero?
      observed  = trend_history(tag).get(at_time).accounts.to_f
