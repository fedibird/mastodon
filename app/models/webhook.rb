...
class Webhook < ApplicationRecord
  ...
  def required_permissions
    events.map { |event| self.class.permission_for_event(event) }
  end
  ...
  def validate_events
    ...
    errors.add(:events, :invalid) if events.any? { |event| EVENTS.exclude?(event) }
  end
  ...
  def validate_permissions
    ...
    missing_permission = required_permissions.compact.any? do |permission|
      !@current_account.user&.role&.can?(permission)
    end

    errors.add(:events, :invalid_permissions) if missing_permission
  end
