...
class Api::V1::Admin::DomainBlocksController < Api::BaseController
  ...
  before_action -> { doorkeeper_authorize! :'admin:read', :'admin:read:domain_blocks' }, only: [:index, :show]
  before_action -> { doorkeeper_authorize! :'admin:write', :'admin:write:domain_blocks' }, except: [:index, :show]
  ...
  def create
    ...
    record = DomainBlock.new(resource_params)
