...
class Api::V1::Admin::EmailDomainBlocksController < Api::BaseController
  ...
  before_action -> { doorkeeper_authorize! :'admin:read', :'admin:read:email_domain_blocks' }, only: [:index, :show]
  before_action -> { doorkeeper_authorize! :'admin:write', :'admin:write:email_domain_blocks' }, except: [:index, :show]
  ...
  def create
    ...
    record = EmailDomainBlock.new
    record.errors.add(:domain, :invalid)
